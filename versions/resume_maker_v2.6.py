#-------------------------------------------------------------------------------------------------------------------------------------------#
# add a microphone selector in the settings (ensure it only shows relevant microphones and not virtual audio devices) -- done
# fix settings and help windown not auto focusing -- done
# fix settings page not working -- done
# allow user to select location for files to be stored, default to a folder in the users download folder named VRM (program name) -- done
# update help menu to say where the files are defaulted too -- done
# in the top menu of the program let ther be an opetion for the file path that all the files will be saved in (defaults to your downloads folder) -- done



# - main fix to be made is voice walkthrough ease of  use. -- make it so that you can stop the voice walkthrough at any time and go back to the home page, this should stop all operationa like tts, transcription, stt, etc. and should reset all the data gathered and should also reset the gui so that it just shows the start page
...
# organise code
#-----------------------------------------------------------------------------------------------------------------------------------------#
# I want to make the voice walkthrough in my gui more seamless and easy to be walked through, i would like to add audio cues so that you know when to speak (e.g. after a beep) and so that you know when an operation is occuring, so it shoudl tell you what it is doing at each point and there should also be within the gui for the voice part of it a indicator that it  is recording like a blinking light, and a more infomrative overall gui for the voice walkthrough as right now it is very minimalistic and doess not give enough information, only rewrite the functions that you change, you should only be mainly updating the voice walkthrough function to make it more comprehensive  : (no blinking light indiactor)
...
def get_autosave_file_path():
    return os.path.join(data_directory, 'autosave.json')
